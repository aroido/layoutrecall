...
### Naming drift observed during the definition run

- **Restore action naming** drifts between `Fix Now`, `Restore Now`, `Apply Layout`, `restore`, and `manual recovery`.
...
- **Surface structure naming** drifted during the definition run because some earlier artifacts still described a five-pane settings window, while the accepted shipped implementation uses **three primary panes** (`Restore`, `Profiles`, `General`) and nests `Shortcuts` + `Diagnostics` inside `General`.
...
| --- | --- | --- |
| Fix Now, Restore Now, manual restore, manual recovery | **Restore Now** | Use one user-facing name for the primary manual recovery action while allowing internal `fixNow` identifiers to remain implementation detail. |
...
| older five-pane settings description | **Three primary settings sections with two advanced sub-sections** | Matches the accepted shipped IA. |
...
- Keep user-facing copy aligned around “Save Current Layout”, “Restore Now”, “Apply Layout”, and “Show Numbers”.
